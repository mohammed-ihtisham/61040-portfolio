...
actions
  setHidePurchases (registry: Registry, flag: Boolean)
    ...
    effects sets the registry’s hidePurchases flag to the provided value
```
...
actions
  register (username: String, password: String): (user: User, token: String)
    requires no user exists with this username
    effects creates a new, unconfirmed user, a confirmation token linked to that
       user with a fresh secret, and returns the user and the secret

  confirm (username: String, token: String)
    requires a user u with u.username = username and u.confirmed = false
      and a token t with t.user = u and t.secret = token
    effects marks the user as confirmed and deletes the matching confirmation token
...
actions
  ...
  revoke (t: Token)
    ...
    effects marks the token inactive
  ...
  authenticateWithToken (t: Token, op: Operation, res: Resource): (user: User)
    ...
    effects returns the token owner
```
...
actions
  ...
  deactivate (link: Link)
    ...
    effects marks the link inactive
```
...
actions
  start (worker: User, project: Project, note: Note): (s: Session)
    ...
    effects creates a new running session starting now with the note and returns it

  end (s: Session, when: DateTime?)
    requires s exists and s.end is unset and (when is unset or when ≥ s.start)
    effects records the end time as the provided time or now

  autoEnd (s: Session, cutoff: DateTime)
    requires s exists and s.end is unset and cutoff ≥ s.start
    effects records the end time as the cutoff and marks the session as auto-ended

  editNote (s: Session, note: Note)
    requires s exists
    effects replaces the session’s note with the provided note
```
...
actions
  create (organizer: User, room: Room, start: DateTime, end: DateTime, title: Title, setup: Setup, coOwners: Set[User]): (b: Booking)
    ...
    effects creates a new active booking with the provided details,
      ensures the organizer is listed among the co-owners, and returns the booking
  ...
  reschedule (b: Booking, room: Room, start: DateTime, end: DateTime)
    ...
    effects updates the booking to use the provided room and time window

  updateDetails (b: Booking, title: Title, setup: Setup, coOwners: Set[User])
    requires b is active
    effects updates the title, setup, and co-owner set, ensuring the organizer remains a co-owner

  cancel (b: Booking)
    requires b is active
    effects marks the booking inactive
```
